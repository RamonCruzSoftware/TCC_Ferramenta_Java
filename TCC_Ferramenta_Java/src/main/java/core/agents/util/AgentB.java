	...
	private class Comportamento extends CyclicBehaviour {
		...
		public void action() {
			try {
				...
				MessageTemplate pedidoDePropostas = new MessageTemplate(
						(MatchExpression) MessageTemplate
								.MatchPerformative(ACLMessage.CFP));
				MessageTemplate propostasRejeitadas = new MessageTemplate(
						(MatchExpression) MessageTemplate
								.MatchPerformative(ACLMessage.REJECT_PROPOSAL));
				MessageTemplate propostasAceitas = new MessageTemplate(
						(MatchExpression) MessageTemplate
								.MatchPerformative(ACLMessage.ACCEPT_PROPOSAL));

				ACLMessage propostasMensagens = myAgent
						.receive(pedidoDePropostas);
				ACLMessage propostaRejeitada = myAgent
						.receive(propostasRejeitadas);
				...
				if (propostaRejeitada != null) {
					...
					System.out.println(this.getAgent().getLocalName()
							+ "Proposta rejeitada, vou enviar outra.");
				}

				if (propostaAceita != null) {
					System.out.println(this.getAgent().getLocalName()
							+ "OK, agora a proposta foi aceita.");
				}
