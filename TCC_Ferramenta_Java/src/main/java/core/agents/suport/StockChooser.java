...
public class StockChooser {
	...
	public StockChooser(ArrayList<Stock> stockList, int userProfile) {
		...
		try {
			if (this.getStockList().size() > 0) {
				...
				for (Stock s : this.getStockList()) {// TODO LOG
														// TODO Retirar esse
														// gato de quebra de
														// laco e resolver o
														// problema
					...
					if (stop == 100)
						break;
					if (s.getCandleSticks() == null)
						s.setCandleSticks(this.getStockDao()
								.getStockPrices_last30(s.getCodeName()));
	...
	public boolean analyzeStock(Stock stockToAnalyze) {
		...
		if (this.getApprovedStockList().size() > 0) {
			for (int i = 0; i < this.getApprovedStockList().size(); i++) {
				correl = statistical.calculeCorrelationCoefficient_30(
						stockToAnalyze.getCandleSticks(), this
								.getApprovedStockList().get(i)
								.getCandleSticks());
				if (correl < 0) {
				} else {
				}
	...
	/**
	 * This method analyze the correlation between stock in an List. Case to
	 * happen some correlation above the limit for user profile, the Stock whose
	 * have negative correlaction is not approved.
	 * 
	 * @return
	 * 
	 *         This method return a list that contain two elements: 0-> Approved
	 *         List; 1-> Refuse List.
	 */
	public ArrayList<ArrayList<Stock>> analyzeStockList() {
		...
		if (this.getStockListTemp_a().size() > 0) {
			...
			// TODO LOG
			System.out.println("Stock inicial aprovado "
					+ stockTemp.getCodeName());
			System.out.println("Lista para avaliacao tem agora "
					+ getStockListTemp_a().size() + " acoes");

			for (int i = 0; i < getStockListTemp_a().size(); i++) {// TODO LOG
				correl_temp = 0;
				System.out.println("===Iniciar analise==");
				System.out.println("Analisando  ["
						+ this.getStockListTemp_a().get(i).getCodeName() + "]");
				System.out.println(" Com "
						+ this.getStockListTemp_a().get(i).getCandleSticks()
								.size() + " candles");
				System.out.println("\n");
				System.out.println(" Stock 1: " + stockTemp.getCodeName()
						+ " com " + stockTemp.getCandleSticks().size()
						+ " candles");
				System.out.println(" Stock 2: "
						+ this.getStockListTemp_a().get(i).getCandleSticks()
								.size() + " candles");

				correl_temp = statistical.calculeCorrelationCoefficient_30(
						stockTemp.getCandleSticks(), this.getStockListTemp_a()
								.get(i).getCandleSticks());
				System.out.println("Correl " + correl_temp);
				if (correl_temp > 0) {// TODO LOG
					System.out.println("::: " + getStockListTemp_a().get(i)
							+ " Refused");
					getRefuseStockList().add(getStockListTemp_a().get(i));
				} else if (correl_temp < 0) {// TODO LOG
					System.out.println("::: " + getStockListTemp_a().get(i)
							+ " Approved");
