...
public class Expert extends Agent {
	...
	private CandleStick requestStocksPrices(Stock stock) 
	{
		...
		try {
			if (expert.yahooFinances.storeCsvCurrentPriceStock(stock
					.getCodeName())) {// TODO Colocar log
				candleStick = expert.yahooFinances.getCurrentValue(stock
						.getCodeName());
			}
			if (!(candleStick.getDate().getTime() == stock.getCandleSticks()
			...
			addBehaviour(new OneShotBehaviour(expert) {
				...
				public void action() {
					try {// TODO log
						...
						// Caso tenha liquidado tudo, volta a solicitar dinheiro
						// pra comprar
	...
	private class StockManager {
		...
		public boolean stockBougth(Stock stockToVerify) {
			ManagedStock managedStockStored = this.managedStockDao
					.getManagedStock(stockToVerify.getCodeName(),
							userIdentifier);
			if (managedStockStored != null)
				return true;
			else
