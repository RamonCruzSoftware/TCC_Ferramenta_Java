package suport.financial.indicators;
...
public class MovingAvarange {
	...
	/**
	 * For to calculate the Simple MovingAvarange pass the values list and
	 * period
	 * 
	 * @param values
	 * @param period
	...
	/**
	 * For to calculate the Exponencial Moving Avarange pass the values last
	 * Avarange in lastMME, Currente price in currentPrice and period in period
	 * 
	 * @param lastMME
	 * @param currentPrice
	 * @param period
	...
	public double simpleAvarange() {
		...
		if (this.values.size() > 0 && this.values.size() >= this.getPeriod()) {
			for (int i = (this.values.size() - 1); i > (this.values.size() - 1 - this
					.getPeriod()); i--) {
	 ...
	 * 
	 * @param value
	 *            insert an list of values always that you want to calculate a
	 *            new value of moving avarange
	 */
	...
	/**
	 * For to recalcute the Exponencial value inform the new values in paramters
	 * 
	 * @param lastMME
	 * @param currentPrice
	 */
