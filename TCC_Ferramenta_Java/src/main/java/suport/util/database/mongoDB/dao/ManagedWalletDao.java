...
public class ManagedWalletDao {
	...
	public ManagedWalletDao() {
		...
		this.setCollection_managedWallet(getDb().getCollection(
				"JADE_managedWallet"));
	...
	public boolean updateManagedWallet(ManagedWallet info) {
		...
		try {
			...
			if (info.getStocksList() != null) {
				...
				for (Stock s : info.getStocksList()) {
					stockListToStore.add(new BasicDBObject("codeName", s
							.getCodeName()).append("sector", s.getSector()));
				}
			}
	...
	public ManagedWallet getManagedWallet(String userIdentifier) {
		...
		try {
			...
			stocksListStored = (ArrayList<BasicDBObject>) managedWalletStored
					.get("stocksList");
			...
			if (managedWalletStored != null) {
				...
				if (stocksListStored != null && stocksListStored.size() > 0) {
					...
					for (BasicDBObject b : stocksListStored) {
						stockList.add(new Stock(b.getString("codeName"), b
								.getString("sector")));
					}
				}
				...
				result.setWalletValue(managedWalletStored
						.getDouble("walletValue"));
				result.setWalletRisck(managedWalletStored
						.getDouble("walletRisck"));
				result.setWalletProfitValue(managedWalletStored
						.getDouble("walletProfitValue"));
				result.setWalletProfitPercent(managedWalletStored
						.getDouble("walletProfitPercent"));
	...
	public void setCollection_managedWallet(
			DBCollection collection_managedWallet) {
