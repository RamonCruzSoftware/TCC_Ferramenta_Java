...
public class YahooFinance implements Runnable {
	...
	public YahooFinance(String mainDirPath, String subDirPath_1,
			String subDirPath_2) {
	...
	public YahooFinance(String mainDirPath, String subDirPath_1,
			String subDirPath_2, ArrayList<Stock> stockList) {
		this.setMainDirPath(mainDirPath);
		this.setSubDirPath_1(subDirPath_1);
			...
			while ((Str = StrR.readLine()) != null) {
				...
				try {
					...
					candleResult = new CandleStick(TableLine[3], TableLine[4],
							TableLine[5], TableLine[6], TableLine[7], date);
	...
	public ArrayList<CandleStick> getHistoricalValue(String stockCodeName) {
		...
		String dirOutputPath = this.getMainDirPath() + this.getSubDirPath_1()
				+ this.getSubDirPath_2() + "/" + stockCodeName;
		String fileOutputPath = dirOutputPath + "/historicalPrice_"
				+ stockCodeName + ".csv";
		...
		try {
			...
			while ((Str = StrR.readLine()) != null) {
				...
				list.add(new CandleStick(TableLine[1], TableLine[2],
	...
	public ArrayList<Stock> loadStocksFromCsv(String stocksCsvfolderPath) {
		...
		for (String s : sectors) {
			try {
				...
				StrR = new BufferedReader(new FileReader(stocksCsvfolderPath
						+ "/" + s));
				TableLine = null;
				Str = null;
				...
				while ((Str = StrR.readLine()) != null) {
					// passando como parametro o divisor ",".
					TableLine = Str.split(",");
					list.add(new Stock(TableLine[0] + ".SA", s.substring(0,
							(s.length() - 4))));
	...
	public boolean storeCsvCurrentPriceStock(String stockCodeName) {
		...
		try {
			...
			fosFileOutputStream.getChannel().transferFrom(rbc, 0,
					Long.MAX_VALUE);
	...
	public boolean storeCsvHistoricalPriceStock(String stockCodeName) {
		...
		String urlPath = "http://real-chart.finance.yahoo.com/table.csv?s="
				+ stockCodeName + "&d="
				+ currentDate.get(Calendar.DAY_OF_MONTH) + "&e="
				+ currentDate.get(Calendar.MONTH) + "&f="
				+ currentDate.get(Calendar.YEAR)
				+ "&g=d&a=0&b=3&c=2000&ignore=.csv";
	...
	public void run() {
		// TODO
		/*
		 * Implementar esse metodo com classe anonima pois o corpo desse metodo
		 * ira variar de acordo com a necessidade.
		 */
